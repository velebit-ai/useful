...
* useful.config - Easily handle configuration files, no matter where they are
* useful.creator - Create objects directly from config files
...
* useful.resource - Single interface to download and parse a resource from multiple locations
...
You can find the module on [`pypi`](https://pypi.org/project/velebit-useful) and install it using

```sh
pip install velebit-useful
```
